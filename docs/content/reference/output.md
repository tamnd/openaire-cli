...
describes what users get. Pick a format with `-o`, or let openaire choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openaire <command> -o table   # aligned columns for reading
openaire <command> -o jsonl   # one JSON object per line, for piping
openaire <command> -o json    # a single JSON array
openaire <command> -o csv     # spreadsheet friendly
openaire <command> -o tsv     # tab-separated
openaire <command> -o url     # just the URL column
openaire <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openaire <command> --fields id,title,url
```
...
```bash
openaire <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openaire <command>            # a table, because this is a terminal
openaire <command> | wc -l    # JSONL, because this is a pipe
```
