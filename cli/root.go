// Package cli assembles the openaire command tree from the openaire
...
// NewApp assembles the kit application from the openaire domain. The
...
//
// To add a command, declare it in openaire/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := openaire.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(openaire.Domain{}).Register(app)
